...
main() {
	...
	case "$RUN_COMMAND" in
	install | upgrade | uninstall | configure)
		"$RUN_COMMAND"
		exit
		;;
	esac
	...
	Linux)
		...
		GNU/Linux)
			...
			for dist in $(. /etc/os-release; echo "$ID_LIKE"); do
				case $dist in debian|ubuntu|rhel|fedora|openwrt)
...
get_release() {
	if [ "$dnsadblock_VERSION" ]; then
		...
		*)
		...
		v=$($cmd "https://api.github.com/repos/dnsadblock/proxy-release/releases/latest" | \
